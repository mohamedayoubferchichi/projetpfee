...
const emptyUserForm = {
  ...
  cin: '', numeroContrat: '', statutCompte: 'NON_VERIFIE'
}
...
export default function AgentPage() {
  ...
  const handleSubmitUser = async (e) => {
    ...
    try {
      const res = await fetch(url, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(userForm) })
      const data = await res.json().catch(()=>null)
  ...
  const handleEditUser = (u) => {
    setUserForm({
      ...
      cin: u.cin || '', numeroContrat: u.numeroContrat || '', statutCompte: u.statutCompte || 'NON_VERIFIE'
    })
  ...
  const filteredUtilisateurs = utilisateurs.filter(u => {
    ...
    if (!t) return true
    return [u.nom, u.email, u.cin, u.numeroContrat].some(v => String(v||'').toLowerCase().includes(t))
  })
          ...
          <div style={{ background: '#f7fafc', borderRadius: '16px', padding: '2rem', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)', border: '1px solid #e2e8f0' }}>
            ...
            <form className="admin-modal-form-grid" onSubmit={handleSaveAgence}>
              ...
              <label style={{ color: '#2d3748', fontWeight: '600' }}>Téléphone
                <input name="telephone" type="text" value={agenceForm.telephone} onChange={e => setAgenceForm(p => ({ ...p, telephone: e.target.value }))} style={{ background: 'white', color: '#1a202c', border: '1px solid #cbd5e0' }} />
              </label>
    ...
    <section className="section admin-block">
      ...
      <div className="admin-table-wrap">
        <table className="admin-data-table">
          <thead>
            <tr>
              <th>CONTRAT</th><th>CIN</th><th>TYPE</th><th>STATUT</th><th>DÉBUT</th><th>FIN</th><th>ACTIONS</th>
            </tr>
          </thead>
          <tbody>
        ...
        <div className="admin-modal-overlay" onClick={() => setIsContratModalOpen(false)}>
          <div className={`admin-modal-card ${editingContratId?'admin-modal-card-blue':'admin-modal-card-success'}`} onClick={e => e.stopPropagation()}>
            ...
            <form className="admin-modal-body admin-modal-form-grid"
              ...
              <label>CIN * <input name="cin" value={contratForm.cin} onChange={e=>setContratForm(p=>({...p,cin:e.target.value}))} required /></label>
    ...
    <section className="section admin-block">
      ...
      <div className="admin-table-wrap">
        <table className="admin-data-table">
          <thead>
            <tr>
              <th>UTILISATEUR</th><th>CONTACT</th><th>CIN</th><th>N° CONTRAT</th><th>STATUT</th><th>ACTIONS</th>
            </tr>
          </thead>
          <tbody>
          ...
          <div className="admin-modal-card" onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', width: '100%', maxWidth: '600px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            ...
            <form className="admin-modal-form-grid" style={{ padding: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}
              ...
              <label style={{ color: '#2d3748', fontSize: '0.85rem', fontWeight: '700' }}>Téléphone
                <input name="telephone" value={userForm.telephone} onChange={e=>setUserForm(p=>({...p,telephone:e.target.value}))} style={{ marginTop: '0.4rem', width: '100%', padding: '0.7rem', border: '1px solid #e2e8f0', borderRadius: '8px', color: '#1a202c' }} />
              </label>
              
              <label style={{ color: '#2d3748', fontSize: '0.85rem', fontWeight: '700' }}>CIN
                <input name="cin" value={userForm.cin} onChange={e=>setUserForm(p=>({...p,cin:e.target.value}))} style={{ marginTop: '0.4rem', width: '100%', padding: '0.7rem', border: '1px solid #e2e8f0', borderRadius: '8px', color: '#1a202c' }} />
              </label>
              
              <label style={{ color: '#2d3748', fontSize: '0.85rem', fontWeight: '700' }}>Numéro contrat
                <input name="numeroContrat" value={userForm.numeroContrat} onChange={e=>setUserForm(p=>({...p,numeroContrat:e.target.value}))} style={{ marginTop: '0.4rem', width: '100%', padding: '0.7rem', border: '1px solid #e2e8f0', borderRadius: '8px', color: '#1a202c' }} />
              </label>
        ...
        <header className="admin-header">
          <div className="admin-header-left">
            <h1 className="admin-header-title">
              {activeSection === 'agence' ? 'Mon agence' : activeSection === 'contrats' ? 'Gestion des contrats' : 'Mes clients'}
            </h1>
          </div>
